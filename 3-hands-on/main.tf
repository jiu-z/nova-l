  ...
  template {
    spec {
      containers {
        ...
        # image = "us-central1-docker.pkg.dev/era-ax/infra/init:latest"
        image = var.container_image
      }
    }
  }
...
resource "google_cloud_run_service_iam_policy" "noauthc" {
  location    = google_cloud_run_service.default.location
  project     = google_cloud_run_service.default.project
  service     = google_cloud_run_service.default.name

  policy_data = data.google_iam_policy.noauthc.policy_data
}
...
#   backend {
#     group = google_compute_region_network_endpoint_group.webserver.id
#   }
#   log_config {
#     enable      = true
#     sample_rate = 1
